...
class Corpus:
    ...
    def __new__(cls, name:str='Corpus 1'):
        """
        ...
        if not cls.__instance:
            ...
            cls.__instance.authors = dict()
            # ID to document mapping
            cls.__instance.id2doc = dict()
            # Author to ID mapping
            cls.__instance.aut2id = dict()
            # Number of documents
            cls.__instance.ndoc = 0
            # Number of authors
            cls.__instance.author_num = 0
    ...
    def __repr__(self):
        ...
        """
        docs = list(self.__instance.id2doc.values())
        return "\n".join(list(map(str, docs)))

    def __str__(self):
        return str(self.__instance.id2doc)
    ...
    def add(self, doc, author):
        ...
        """
        if author not in self.__instance.aut2id:
            self.__instance.author_num += 1
            self.__instance.authors[self.__instance.author_num] = Author(author)
            self.__instance.aut2id[author] = self.__instance.author_num
        self.__instance.authors[self.__instance.aut2id[author]].add(doc.text)

        self.__instance.ndoc += 1
        self.__instance.id2doc[self.__instance.ndoc] = doc
    ...
    def corpus_stats(self) -> None:
        ...
        """
        full_corpus = self.__instance.id2doc.values()
    ...
    def concat_data(self) -> None:
        ...
        if self.__concated_text is None:
            self.__concated_text = "".join([doc.text for doc in self.__instance.id2doc.values()])
    ...
    def stats(self) -> pd.DataFrame:
        ...
        ids = list()
        counter = 0
        for doc in self.__instance.id2doc:
            splited_text.append(self.clean_text(self.__instance.id2doc[doc].text).split(' '))
    ...
    def get_docs_by_author(self, author: str) -> list:
        ...
        Returns:
            A list of documents.
        """
        return self.__instance.authors[self.__instance.aut2id[author]].docs
    ...
    def get_all_docs(self) -> list:
        ...
        Returns:
            A list of documents.
        """
        return list(self.__instance.id2doc.values())
